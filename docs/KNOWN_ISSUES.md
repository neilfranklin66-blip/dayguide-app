...
- **Likely dependency:** None blocking. The affected screens — itinerary rows and geographic choice — are still recorded as legacy and not approved for the current design, but the miles rule is permanent, so the work survives their redesign. Routing the values through the shared formatter means the new design inherits the behaviour rather than reimplementing it.
- **Recommended next action:** Route `TimelineItemRow.jsx:37`, `geography.fromStart` and `geography.toLater` through `formatNearbyDistance`, and apply the very-short-distance rule specified in `DESIGN_BASELINE.md` v1.12. The Product Owner has confirmed the miles rule is permanent for these screens and does not depend on their redesign. Do not change the formatter.
