...
**Very short distances.** Where a measured distance is small enough that the displayed figure would round to zero — roughly 80 metres in English, roughly 50 metres in the kilometre locales — DayGuide does not display a number. It says **You’re already close** instead. This applies wherever a distance is shown: the Nearby card, the itinerary row, and the geographic choice screen.

The threshold is derived from the display rule rather than chosen: it is the point below which the formatted value reads 0.0. It is therefore not a fixed distance. English converts to miles, so the figure reads 0.0 below roughly 80 metres; the kilometre locales apply no conversion, so it reads 0.0 only below roughly 50 metres. Applying the English figure uniformly would suppress a usable number in Spanish, French, Vietnamese and Chinese. At that range the figure is also smaller than the accuracy of the underlying coordinates, and shorter than any walking route. Above it, a small distance is still useful and must be shown — 150 metres is a two-minute walk and should read as a number.
