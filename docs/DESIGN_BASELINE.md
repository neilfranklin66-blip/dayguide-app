...
**Very short distances.** Where a measured distance is small enough that the displayed figure would round to zero — roughly 80 metres — DayGuide does not display a number. It says **You’re already close** instead. This applies wherever a distance is shown: the Nearby card, the itinerary row, and the geographic choice screen.

The threshold is derived from the display rule rather than chosen: it is the point below which the formatted value reads 0.0. At that range the figure is also smaller than the accuracy of the underlying coordinates, and shorter than any walking route. Above it, a small distance is still useful and must be shown — 150 metres is a two-minute walk and should read as a number.
...
| Version | Date | Change |
| --- | --- | --- |
| 1.11 | 1 September 2026 | Recorded the intended, not-yet-implemented very-short-distance rule: where the displayed figure would round to zero — roughly 80 metres — DayGuide shows **You’re already close** rather than a number or a blank, wherever a distance appears, including the Nearby card, the itinerary row, and the geographic choice screen. The threshold is derived from the display rule rather than chosen. The wording in Spanish, French, Vietnamese and Chinese remains undecided. This is a documentation-only direction; it makes no application-source, deployment, credential, or provider change. |
