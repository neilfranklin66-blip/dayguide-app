import { PACKET155_CALIBRATION_PLAN } from './packet155Approval';

export const PACKET155_RUN_ACKNOWLEDGEMENT =
  'PACKET155_MAXIMUM_24_PROVIDER_EVENTS_APPROVED';
export const PACKET155_CANONICAL_PRODUCTION_ORIGIN =
  'https://ubiquitous-melomakarona-874d9c.netlify.app';

export const PACKET155_RUN_ERROR = {
  INVALID_ACKNOWLEDGEMENT: 'PACKET155_ACKNOWLEDGEMENT_REQUIRED',
  INVALID_PREVIEW_ORIGIN: 'PACKET155_PREVIEW_ORIGIN_INVALID',
  INVALID_BATCH_SELECTION: 'PACKET155_BATCH_SELECTION_INVALID',
  AUTHENTICATION_FAILED: 'PACKET155_AUTHENTICATION_FAILED',
  PREVIEW_UNAVAILABLE: 'PACKET155_PREVIEW_UNAVAILABLE',
  PREVIEW_RATE_LIMITED: 'PACKET155_PREVIEW_RATE_LIMITED',
  PROVIDER_REJECTED: 'PACKET155_PROVIDER_REJECTED',
  INVALID_RESPONSE: 'PACKET155_RESPONSE_INVALID',
};
...
const normalizePreviewOrigin = value => {
  ...
  try {
    ...
    throw errorWithCode(PACKET155_RUN_ERROR.INVALID_PREVIEW_ORIGIN);
  }
  if (
    url.protocol !== 'https:' ||
    url.username ||
    ...
    url.origin === PACKET155_CANONICAL_PRODUCTION_ORIGIN
  ) {
    throw errorWithCode(PACKET155_RUN_ERROR.INVALID_PREVIEW_ORIGIN);
  }
  return url.origin;
...
const normalizeBatchIndexes = value => {
  ...
  if (
    ...
    indexes.some(
      index =>
        index < 0 ||
        index >= PACKET155_CALIBRATION_PLAN.batches.length,
    )
  ) {
    throw errorWithCode(PACKET155_RUN_ERROR.INVALID_BATCH_SELECTION);
  }
  return [...indexes].sort((left, right) => left - right);
};

export function createPacket155RunSelection({
  previewOrigin,
  acknowledgement,
  batchIndexes,
} = {}) {
  if (acknowledgement !== PACKET155_RUN_ACKNOWLEDGEMENT) {
    throw errorWithCode(
      PACKET155_RUN_ERROR.INVALID_ACKNOWLEDGEMENT,
    );
  }
  const normalizedIndexes = normalizeBatchIndexes(batchIndexes);
  const batches = normalizedIndexes.map(
    index => PACKET155_CALIBRATION_PLAN.batches[index],
  );
  ...
  return {
    ...
    fullExerciseMaximumProviderRequests:
      PACKET155_CALIBRATION_PLAN.maximumBillableEvents,
...
const parsePayload = async response => {
  try {
    ...
    throw errorWithCode(PACKET155_RUN_ERROR.INVALID_RESPONSE);
  }
};

const statusError = (response, payload) => {
  if (response.status === 429) {
    return PACKET155_RUN_ERROR.PREVIEW_RATE_LIMITED;
  }
  if (response.status === 401 || response.status === 403) {
    return PACKET155_RUN_ERROR.AUTHENTICATION_FAILED;
  }
  if (
    payload?.status === 'REQUEST_DENIED' ||
    payload?.status === 'AUTH_REQUIRED'
  ) {
    return PACKET155_RUN_ERROR.PROVIDER_REJECTED;
  }
  ...
  if (
    ...
    return PACKET155_RUN_ERROR.PREVIEW_UNAVAILABLE;
  }
  if (!response.ok) {
    return PACKET155_RUN_ERROR.PREVIEW_UNAVAILABLE;
  }
...
export async function runPacket155Calibration({
  selection,
  getIdToken,
  ...
  if (
    ...
    throw errorWithCode(PACKET155_RUN_ERROR.AUTHENTICATION_FAILED);
  }
  const output = [];
  ...
  for (let offset = 0; offset < selection.batches.length; offset += 1) {
    ...
    try {
      ...
      throw errorWithCode(PACKET155_RUN_ERROR.PREVIEW_UNAVAILABLE);
    }
    if (response.status === 429) {
      throw errorWithCode(
...
const packet155CalibrationRunner = {
...
export default packet155CalibrationRunner;
