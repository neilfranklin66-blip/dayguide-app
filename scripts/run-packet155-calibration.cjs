...
const main = async () => {
  ...
  } = require('../src/routing/packet155CalibrationRunner');
  ...
  try {
    const selection = createPacket155RunSelection({
      previewOrigin: process.env.PACKET155_PREVIEW_ORIGIN,
      acknowledgement: process.env.PACKET155_RUN_ACKNOWLEDGEMENT,
      batchIndexes: process.env.PACKET155_BATCH_INDEXES,
    });
    const credential = await signInAnonymously(auth);
    anonymousUser = credential.user;
    const output = await runPacket155Calibration({
      selection,
      getIdToken: () => anonymousUser.getIdToken(),
      fetchImpl: globalThis.fetch,
    });
    process.stdout.write(
      `PACKET155_SANITIZED_EVIDENCE_BEGIN\n${JSON.stringify(output, null, 2)}\nPACKET155_SANITIZED_EVIDENCE_END\n`,
    );
  } finally {
    if (anonymousUser) {
      try {
...
main().catch(error => {
  const code =
    ...
    /^PACKET155_[A-Z0-9_]+$/.test(error.message)
      ? error.message
      : 'PACKET155_RUN_FAILED';
  stableFailure(code);
});
